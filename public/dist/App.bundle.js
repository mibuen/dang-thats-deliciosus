  ...
  _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
    ...
    markers.forEach(function (marker) {
      return marker.addListener('click', function () {
        var html = '\n        <div class="popup">\n          <a href="/store/' + this.place.slug + '">\n            <img src="/uploads/' + (this.place.photo || 'store.png') + '" alt=' + this.place.storeName + '"/>\n            <p>' + this.place.storeName + ' - ' + this.place.location.storeAddress + ' </p>\n          </a>\n        </div>\n        ';
...
function searchResultsHTML(stores) {
  return stores.map(function (store) {
    return '<a href="/store/' + store.slug + '" class="search__result"<strong>' + store.storeName + '</strong></a>';
  }).join('');
}
...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    _axios2.default.get('/api/search?q=' + searchInput.value).then(function (res) {
      ...
      searchResults.innerHTML = _dompurify2.default.sanitize('<div class="search__result">' + searchInput.value + ' not found \uD83D\uDC4E</div>');
    }).catch(function (err) {
      console.log(err);
    });
  });
